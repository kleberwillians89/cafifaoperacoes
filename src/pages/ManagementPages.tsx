...
function Modal({ title, children, onClose }: { title: string; children: ReactNode; onClose: () => void }) { return <div className="modal-layer" onMouseDown={onClose}><div className="modal-card" onMouseDown={(e) => e.stopPropagation()}><header><h2>{title}</h2><button onClick={onClose}>×</button></header>{children}</div></div> }
function Actions({ onEdit, onDelete }: { onEdit: () => void; onDelete: () => void }) { return <div className="row-actions"><button onClick={onEdit}><Edit3 size={15}/></button><button onClick={onDelete}><Trash2 size={15}/></button></div> }
...
export function AreasPage() {
  ...
  return <><PageHeader eyebrow="Estrutura" title="Áreas" description="Frentes operacionais e responsáveis." actions={<button className="primary-button" onClick={() => setEditing(null)}><Plus size={16}/> Nova área</button>}/><div className="entity-grid">{data?.areas.map((area) => <article className={`entity-card ${!area.active ? 'inactive' : ''}`} key={area.id}><div><span className="eyebrow">Área operacional</span><h2>{area.name}</h2><p>{area.description || 'Sem descrição.'}</p><small>{data.tasks.filter((t) => t.area_id === area.id).length} tarefas · {area.active ? 'Ativa' : 'Inativa'}</small></div><Actions onEdit={() => setEditing(area)} onDelete={async () => { await archiveArea(area.id); invalidate(client) }}/></article>)}</div>{editing !== undefined && <Modal title={editing ? 'Editar área' : 'Nova área'} onClose={() => setEditing(undefined)}><EntityForm initial={editing ?? {}} fields={['name','description']} onSubmit={(value) => save.mutate(value as unknown as Partial<Area> & { name: string })}/></Modal>}</>
}
